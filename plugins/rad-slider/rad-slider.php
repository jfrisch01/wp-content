...
function rad_slider_image(){
	add_image_size( 'rad-slider', 480, 281, true );	
}
add_action('init', 'rad_slider_image');


/**
 * HTML output of the slider - show a list of up to 5 slides
 */

function rad_slider(){
	$slide_query = new WP_Query( array(
		'post_type' => 'slide', //registered at the top of this plugin
		'showposts' => 5, //maximum
		'nopaging'  => true, //ignore pagination controls on archive pages
	) );
	//custom loo
...
function rad_slider_scripts(){
	...
	wp_enqueue_style( 'rad-slider-style', $css_path );
	...
	wp_enqueue_script( 'rs-custom', $custom_path, 'responsiveslides' );
